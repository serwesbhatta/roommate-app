...
const PersonalInfoTab = ({ profile, user_id, refreshProfile }) => {
  ...
  // When profile prop changes, update the form state and photo preview.
  useEffect(() => {
    if (profile) {
      setFormValues({
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      await dispatch(updateUserPassword({user_id,updateData})).unwrap(); // Use unwrap to catch any errors
  ...
  // Submit handler that includes both text fields and photo
  const handleSave = async (updatedData) => {
    if (!validateProfileForm(updatedData)) return;

    const fd = new FormData();
    // use msu_email, not msu_id
    ...
    try {
      await dispatch(
        updateUserProfile({ userId: user_id, profileData: fd })
      ).unwrap();
      refreshProfile();
      setEditMode(false);
      setPhotoFile(null);
          ...
          {!editMode ? (
            <Grid container spacing={3}>
              ...
              <InfoItem
                label="Move In Date"
                value={
                  formValues.move_in_date
                    ? new Date(formValues.move_in_date).toLocaleDateString()
                    : ''
                }
              />
            ...
            <>
              {/* Photo upload section in its own container */}
