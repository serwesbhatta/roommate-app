...
const EventCard = ({ event, onEditClick, onCancelClick }) => {
  ...
  const nameLabel =
    event.requested_by === currentUserId ? "You" : event.requested_user_name;

  const isCurrentUser =
    event.requested_by === currentUserId || event.organizer === "Current User";
    ...
    <Card sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <CardContent sx={{ flexGrow: 1 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
          ...
          <Typography variant="body2">
            {event.event_start
              ? `${formatTime(event.event_start)} - ${formatTime(
                  event.event_end
                )}`
              : event.time}
          </Typography>
